// forked from http://studio.sketchpad.cc/sp/pad/view/ro.9qzGsMajWwYPo/rev.508

static final int numParticles=100;
    Particle[] particles = new Particle[numParticles ];
    float v = 1/30;
 
 static final double GRAVITY = 128;
    static final float BOUNCE_DAMPENING = 1/64;
...
public void draw() {
  ...
  if (vectors)
    drawMotionVectorsImmediate( vScale*1.4);
...
private void drawParticles() {
 ...
 for (int i = 0; i < numParticles; i++) {
            ...
            int d = parseInt(particles[i].radius * 2);
	 				...
	 				text("ايه بقي", (float) particles[i].posX,particles[i].posY)
        }
...
private void updateParticles() {
    ...
    class Particle {
        ...
        public float getVelocity() {
            return sqrt(vX/2 * vX/2 + vY/2 * vY/2);
        }
